...
def generate_launch_description():
    ...
    repo_directory = get_package_share_directory('stage_utils')
    
    repo_dir = os.path.join(repo_directory, 'launch')
    ...
    name="map_to_odom",
    arguments=["-7", "-7", "0", "0.7854", "0", "0", "map", "odom"]
    )
    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(repo_dir, 'rviz.launch.py')),
            condition=IfCondition(rviz),
            launch_arguments={'namespace': namespace,
                              'use_sim_time': use_sim_time,
                              'config': world}.items()),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(repo_dir, 'stage.launch.py')),
